  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const mappedResult: DetectionResult = {
        ...
        riskLevel: data.risk_level === 'High' ? 'High' : data.risk_level === 'Medium' ? 'Medium' : 'Low',
      ...
      showToast(
        mappedResult.prediction === 'spam' ? 'Spam detected — review the reasons below.' : 'This email looks safe.',
        mappedResult.prediction === 'spam' ? 'error' : 'success'
      );
    } catch (error) {
      showToast(error instanceof Error ? error.message : 'Unable to analyze email', 'error');
    } finally {
      setLoading(false);
    }
  };
